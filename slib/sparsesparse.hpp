#ifndef SPARSESPARSE_HPP
#define SPARSESPARSE_HPP
...
class CooArray
{
	...
	std::array<std::string, RANK> const dims;				// Name of each dimension (OPTIONAL)


	typedef CooArray<IndexT, ValueT, RANK> ThisCooArray;
public:
...
public:
	...
	CooArray(CooArray &&other) :
		shape(other.shape), dims(other.dims),
	...
	void operator=(ThisCooArray const &other) {
		shape = other.shape;
		dims = other.dims;
		indices = other.indices;
		vals = other.vals;
	...
	void clear() {
		for (int k=0; k<RANK; ++k) indices[k].clear();
		vals.clear();
	}

	void reserve(size_t size) {
		for (int k=0; k<RANK; ++k) indices[k].reserve(size);
		vals.reserve(size);
	}
